  ...
  const sendPayment = async () => {
    ...
    try {
      ...
      const client = createWalletClient({
        account: address,
        chain: tempoModerato,
        transport: withFeePayer(custom(provider), http('https://sponsor.moderato.tempo.xyz')),
      }).extend(tempoActions())

      const memo = link.memo || `pay-${link.slug}`
      const memoHex = pad(stringToHex(memo.slice(0, 32)), { size: 32 })
    ...
    <div className="min-h-screen flex flex-col bg-[#f6f9fc]">
      <header className="border-b bg-white">
        <div className="max-w-[480px] mx-auto px-6 py-4 flex items-center justify-between">
          <Link href="/" className="text-xl font-bold text-[#32325d] no-underline">
            Parrot Pay
          </Link>
      ...
      <main className="flex-1 flex items-center justify-center py-8 px-4">
        <div className="w-full max-w-[480px] bg-white rounded-xl shadow-sm border border-[#e6e9ec] overflow-hidden">
